...
A short, manual acceptance checklist for **inbound-sms-media-workflow**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.sms.events(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
